...
4. 分为三个板块：「🔥 AI 行业动态」、「🚀 Product Hunt 新品」和「⭐ GitHub Trending」
...
export async function generateSummary(
  ...
  const ghText = trendingRepos
    .map(
      (r, i) =>
        `[${i + 1}] ${r.fullName} - ${r.description} (⭐${r.stars} +${r.todayStars} today)\n    Language: ${r.language || "N/A"}\n    ${r.url}`
    )
    .join("\n\n");

  const userPrompt = `以下是 ${date} 的 AI 相关新闻、Product Hunt 发布和 GitHub Trending 数据，请生成今日 AI 日报。
